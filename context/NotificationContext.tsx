...
import { NotificationMessage, PendingCountUpdate } from '../lib/signalr/signalRService';
  ...
  useEffect(() => {
    ...
    const unsubscribe = signalR.onNewOrder((message: NotificationMessage) => {
      ...
      if (showAlerts) {
        Alert.alert(
          '🆕 New Order!',
          `New order from ${message.customerName || 'Customer'}\nItems: ${message.itemCount || 0}\nAmount: ${message.totalAmount ? (Math.ceil(message.totalAmount / 1000) * 1000).toLocaleString() : '0'} VND`,
          [{ text: 'OK' }]
        );
      }
    });
