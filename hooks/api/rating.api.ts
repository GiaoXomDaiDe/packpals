...
import { CreateRatingApiRequest, Rating, RatingQuery, UpdateRatingApiRequest } from '../../types/rating.types'
...
export class RatingAPI {
   ...
   * Create a new rating
   */
  async createRating(data: CreateRatingApiRequest): Promise<string> {
    try {
   ...
   */
  async updateRating(data: UpdateRatingApiRequest): Promise<string> {
    try {
  ...
  async getUserRatings(renterId: string, query: RatingQuery = {}): Promise<Rating[]> {
    try {
      ...
      if (response.data.code === 'SUCCESS') {
        console.log('🏷️ Ratings data:', response.data.data)
        // Handle both paginated and simple array responses - Fixed after backend changes
